...
@Component({
    selector: 'app-my-driver-requests',
    standalone: true,
    imports: [CommonModule, SCTTableModule, TranslateModule, FormsModule],
    templateUrl: './my-driver-requests.component.html'
})
export class MyDriverRequestsComponent {

    driverRequests: DriverRequest[] = [];

    columnConfig: ColumnsConfig[] = [
        { key: 'name', name: this.translate.instant('driver_requests.name'), type: 'string' },
        { key: 'email', name: this.translate.instant('driver_requests.email'), type: 'string' },
    ...
    edit(request: DriverRequest) {
        this.router.navigate(['/my-driver-requests/edit', request.id]);
    }
    ...
    deleteRequest(request: DriverRequest) {
        ...
        this.driverRequestsService.deleteMyRequest(request.id).subscribe({
            next: () => {
    ...
    addNew() {
        this.router.navigate(['/my-driver-requests/create']);
    }
}
